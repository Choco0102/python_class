# python 중급반 8주차 정규수업(10:00~12:00) mission
'''
[수업을 시작하기 전에!]
1. 카카오톡 질문 단체톡방 ON!
...
class ShotSprite(Sprite):
    ...
    def handleCollision(self, other):
        if type(other) == AlienSprite:      # other의 자료형이 AlienSprite일 때, 둘다 삭제할 것(removeSprite())
...
class GalagaGame():
    ...
    # __init__(): 생성자 메서드
    def __init__(self, master):         #Tk() 객체가 master로 바뀌게 된다.(280번 언저리 확인)
        # 객체 생성
        self.master = master
        self.sprites = []
        ...
        # 이미지 생성
        self.alienImage = PhotoImage(r"C:\python_deep\python_level_middle\4th_week\image\image\alien.png")
        self.shipImage = PhotoImage(r"C:\python_deep\python_level_middle\4th_week\image\image\starship.png")
        self.shotImage = PhotoImage(r"C:\python_deep\python_level_middle\4th_week\image\image\fire.png")
        # 게임실행 여부 & sprite 초기화
        self.running = True
        self.initSprites()
        # 이벤트 처리(7가지 이벤트 처리)
    ...
    def startGame(self, event):
        self.sprites.clear()    # sprites 객체 전체 삭제
        self.initSprties()      # sprites 객체 초기화

    # endGame() 메서드: 게임 종료(Game Over의 조건을 충족했을 때 실행되는 메서드)
    def endGame(self):
        self.running = False    # 게임 실행여부 "False"로 설정
        sys.exit()              # 프로그램 종료

    # removeSprite() 메서드: 스프라이트를 리스트에서 삭제
    def removeSprite(self, sprite):
        if sprite in self.sprites:      # 입력된 sprite가 self.sprites 리스트에 있다면,
            self.sprites.remove(sprite) # self.sprites에서 sprite 제거
            del sprite                  # sprite 제거

    # fire() 메서드: 포탄 발사
    def fire(self):
        self.shot = ShotSprite(self, self.ShotImage, self.starship.get.x() +20, self.starship.get.y() -30)    # 포탄 객체 shot 생성
